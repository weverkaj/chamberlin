...
buckets = read.csv("../data/csv/buckets.csv") %>% mutate(bucket = as.character(bucket))
...
standard_lms = standard.lm(n2o_standards, std_field = "std.ppm", peak_field = "area")
...
a_s = merge(n2O_measurements, standard_equations, by.x = "date.analysis", by.y = "date") %>% 
  ...
  select(date.analysis, date.collection, treatment.type, site.id, bucket, area, Time, QC...Y.N., concentration_ppm) %>%
  group_by(date.collection, site.id, treatment.type, bucket) %>%
  mutate(pressure..atm. = 1)
...
```{r}
#using lms
d_m = n2O_measurements %>%
  group_by(date.analysis) %>%
  nest %>%
  inner_join(standard_lms, by = c("date.analysis" = "date")) %>%
  mutate(concentration_ppm = map2(fit, data.x, 
                                  ~as.data.frame(predict(.x, .y, interval = "confidence")))) %>%
  unnest(data.x, concentration_ppm) %>%
  select(date.analysis, date.collection, treatment.type, site.id, bucket, area, Time, QC...Y.N., concentration_ppm = fit1, lwr, upr) %>%
  group_by(date.collection, site.id, treatment.type, bucket) %>%
  mutate(pressure..atm. = 1)

#add in bucket and climate data

d_m = left_join(d_m, buckets, by = "bucket") %>%
  left_join(high, by = c("date.collection", "site.id", "treatment.type"))
...
```{r}
all_concentrations = d_m %>% mutate(moles = ((pressure..atm. * volume)/(0.082057 * kelvin)) * concentration_ppm/1000000,
